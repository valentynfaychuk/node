defmodule ConsensusKV do
    ...
    def kv_put(key, value \\ "", opts \\ %{}) do
        ...
        {old_value, exists} = case RDB.transaction_get_cf(db.rtx, db.cf.contractstate, key) do
            {:ok, nil} -> {"", false}
            {:ok, value} -> {value, true}
        end
    ...
    def kv_get(key, opts \\ %{}) do
        db = Process.get({RocksDB, :ctx})
        case RDB.transaction_get_cf(db.rtx, db.cf.contractstate, key) do
            {:ok, nil} -> nil
            {:ok, value} ->
                cond do
    ...
    def kv_get_next(prefix, key, opts \\ %{}) do
        db = Process.get({RocksDB, :ctx})
        {:ok, it} = RDB.transaction_iterator_cf(db.rtx, db.cf.contractstate)
    ...
    def kv_get_prev(prefix, key, opts \\ %{}) do
        ...
        {:ok, it} = RDB.transaction_iterator_cf(db.rtx, db.cf.contractstate)
        seek_string = prefix <> key
        seek_res = RDB.iterator_move(it, {:seek_for_prev, seek_string})
        case seek_res do
    ...
    def kv_set_bit(key, bit_idx) do
      db = Process.get({RocksDB, :ctx})
      {old_value, exists} = case RDB.transaction_get_cf(db.rtx, db.cf.contractstate, key) do
          {:ok, nil} -> {<<0::size(SolBloom.page_size())>>, false}
          {:ok, value} -> {value, true}
      end
