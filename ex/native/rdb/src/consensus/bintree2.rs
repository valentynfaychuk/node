...
mod tests {
    ...
    fn test_unified_proof_logic() {
        ...
        assert_eq!(Hubt2::verify(&proof_inc, None, k1.clone(), v1.clone()), VerifyStatus::Included);
        ...
        assert_eq!(Hubt2::verify(&proof_mis, None, k1.clone(), v1_fake), VerifyStatus::Mismatch);
        ...
        let res = Hubt2::verify(&proof_non, None, k_missing.to_vec(), v1.clone());
        ...
        assert_eq!(Hubt2::verify(&proof_non, None, k_missing, v1.clone()), VerifyStatus::NonExistence);
    }
    ...
    fn insert_one(h: &mut Hubt2, k: &[u8], v: &[u8]) {
        h.batch_update(vec![Op::Insert(None, k.to_vec(), v.to_vec())]);
    }

    fn insert_two(h: &mut Hubt2, k0: &[u8], v0: &[u8], k1: &[u8], v1: &[u8]) {
        h.batch_update(vec![
    ...
    fn delete_one(h: &mut Hubt2, k: &[u8]) {
        h.batch_update(vec![Op::Delete(None, k.to_vec())]);
    }

    fn insert_two_none(h: &mut Hubt2, k0: &[u8], v0: &[u8], k1: &[u8], v1: &[u8]) {
        h.batch_update(vec![
            Op::Insert(None, k0.to_vec(), v0.to_vec()),
            Op::Insert(None, k1.to_vec(), v1.to_vec()),
        ]);
    ...
    fn test_deletion_and_upsert_logic() {
        ...
        assert_eq!(Hubt2::verify(&hubt.prove(None, k.clone()), None, k.clone(), v1.clone()), VerifyStatus::Included);

        // 2. Delete
        hubt.batch_update(vec![Op::Delete(None, k.clone())]);
        assert_eq!(Hubt2::verify(&hubt.prove(None, k.clone()), None, k.clone(), v1.clone()), VerifyStatus::NonExistence);
        ...
        assert_eq!(Hubt2::verify(&hubt.prove(None, k.clone()), None, k.clone(), v2.clone()), VerifyStatus::Included);
    }
    ...
    fn attack_namespace_swap_allows_false_inclusion_even_with_multiple_leaves() {
        ...
        assert!(Hubt2::verify_integrity(&proof_a));
        assert_eq!(
            Hubt2::verify(&proof_a, Some(ns_a.clone()), k0.clone(), v0.clone()),
        ...
        let status = Hubt2::verify(&forged, Some(ns_b), k0, v0);
        assert_ne!(
            status,
            VerifyStatus::Included,
    ...
    fn attack_proof_path_bitflip_can_turn_invalid_into_false_nonexistence() {
        ...
        assert!(Hubt2::verify_integrity(&proof0));
        ...
        assert_eq!(
            Hubt2::verify(&proof0, None, k1.clone(), v1.clone()),
            VerifyStatus::Invalid
        );
        ...
        let status = Hubt2::verify(&forged, None, k1, v1);
        assert_eq!(
            status,
            VerifyStatus::Invalid,
